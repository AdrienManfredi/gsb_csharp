...
CREATE TABLE OFFRIR
(    
	 ID_OFFRIR INT IDENTITY(1,1),       
     RAP_NUM INT  not null  ,
     MED_DEPOTLEGAL varchar(50)  not null  ,
     OFF_QTE INT not null,
     CONSTRAINT PK_OFFRIR PRIMARY KEY ( ID_OFFRIR),
...
go
/* -----------------------------------------------------------------------------
      TABLE : ARCHIVAGE_RAPPORT_VISITE
----------------------------------------------------------------------------- */

CREATE TABLE ARCHIVAGE_RAPPORT_VISITE
(
	ARC_RAP_NUM INT IDENTITY (1,1),
	ARC_VIS_MATRICULE CHAR(4)  not null,
    ARC_PRA_NUM int  not null,
    ARC_RAP_DATE DATE not null  ,
    ARC_RAP_BILAN varchar(255) not null  ,
    ARC_RAP_MOTIF varchar(255) not null,
    CONSTRAINT PK_ARCHIVAGE_RAPPORT_VISITE PRIMARY KEY (ARC_RAP_NUM)
)
go
/* -----------------------------------------------------------------------------
      TABLE : ARCHIVAGE_OFFRIR
----------------------------------------------------------------------------- */

CREATE TABLE ARCHIVAGE_OFFRIR
(
	ARC_RAP_NUM INT not null,
    ARC_MED_DEPOTLEGAL varchar(50) not null,
    ARC_OFF_QTE INT not null,
    CONSTRAINT PK_ARCHIVAGE_OFFRIR PRIMARY KEY (ARC_RAP_NUM, ARC_MED_DEPOTLEGAL),
	CONSTRAINT FK_ARCHIVAGE_OFFRIR_ARCHIVAGE_RAPPORT_VISITE FOREIGN KEY (ARC_RAP_NUM) REFERENCES ARCHIVAGE_RAPPORT_VISITE (ARC_RAP_NUM)
)
go
